...
const babelTransformTypescript = require('@babel/plugin-transform-typescript');
...
const ast = require('@mrleebo/prisma-ast');
...
const spawn__default = /*#__PURE__*/_interopDefaultLegacy(spawn);
const chalk__default = /*#__PURE__*/_interopDefaultLegacy(chalk);
const fs__namespace = /*#__PURE__*/_interopNamespace(fs);
const path__namespace = /*#__PURE__*/_interopNamespace(path);
const path__default = /*#__PURE__*/_interopDefaultLegacy(path);
const username__default = /*#__PURE__*/_interopDefaultLegacy(username);
const babel__namespace = /*#__PURE__*/_interopNamespace(babel);
const babelTransformTypescript__default = /*#__PURE__*/_interopDefaultLegacy(babelTransformTypescript);
...
const babelParser__namespace = /*#__PURE__*/_interopNamespace(babelParser);
...
const ast__namespace = /*#__PURE__*/_interopNamespace(ast);
const which__default = /*#__PURE__*/_interopDefaultLegacy(which);
const pluralize__default = /*#__PURE__*/_interopDefaultLegacy(pluralize);
...
class ConflictChecker extends stream.Transform {
  constructor(options) {
    super({
      objectMode: true
    });
  ...
  _transform(file, _encoding, cb) {
    ...
    const filePath = path__namespace.resolve(file.path);
  ...
  checkDiff(file) {
    return __async$e(this, null, function* () {
      ...
      const oldFileContents = fs__namespace.readFileSync(path__namespace.resolve(file.path)).toString();
      const diff$1 = diff.diffLines(oldFileContents, newFileContents);
      const conflict = diff$1.some((line) => line.added || line.removed);
      if (conflict) {
        let answer = null;
        do {
          answer = yield enquirer__default.prompt({
            type: "select",
            name: "action",
  ...
  printDiff(diff) {
    ...
    diff.forEach((line) => {
      ...
      if (line.added) {
        console.log(chalk__default.green(`+ ${value}`));
      } else if (line.removed) {
        console.log(chalk__default.red(`- ${value}`));
      } else {
        console.log(value);
      }
    });
  ...
  fileStatusString(file, status, dryRun = false) {
    ...
    switch (status) {
      case "create":
        statusLog = chalk__default.green(`${dryRun ? "Would create" : "CREATE"}   `);
        break;
      case "overwrite":
        statusLog = chalk__default.cyan(`${dryRun ? "Would overwrite" : "OVERWRITE"}   `);
        break;
      case "skip":
        statusLog = chalk__default.blue("SKIP     ");
        break;
      case "identical":
        statusLog = chalk__default.gray("IDENTICAL");
    }
...
function readdirRecursive(root, filter, dir = "") {
  return __async$d(this, null, function* () {
    ...
    const dirStats = yield fs$1.promises.stat(absoluteDir);
    if (dirStats.isDirectory()) {
      let entries = yield fs$1.promises.readdir(absoluteDir);
      if (filter) {
...
function getProjectRootSync() {
  return path__namespace.dirname(getConfigSrcPath());
}
function getConfigSrcPath() {
  const jsPath = path__namespace.resolve(path__namespace.join(process.cwd(), "next.config.js"));
  return jsPath;
}
const customTemplatesBlitzConfig = (isTypeScript, customTemplatesPath = "", codemod = false) => __async$c(void 0, null, function* () {
  ...
  const userConfigModuleSource = fs__namespace.readFileSync(blitzServerPath, { encoding: "utf-8" });
  const userConfigModule = j__default(userConfigModuleSource, { parser: customTsParser });
  const program = userConfigModule.get();
  const cliConfigDeclaration = userConfigModule.find(j__default.ExportNamedDeclaration, {
    declaration: {
  ...
  if (!cliConfigDeclaration) {
    const config = j__default.identifier("cliConfig");
    const configVariable = j__default.variableDeclaration("const", [
      j__default.variableDeclarator(
        config,
        j__default.objectExpression([
          j__default.objectProperty(j__default.identifier("customTemplates"), j__default.literal(customTemplatesPath))
        ])
      )
    ]);
    if (isTypeScript) {
      const type = j__default.tsTypeAnnotation(j__default.tsTypeReference(j__default.identifier("BlitzCliConfig")));
      const declaration = configVariable == null ? void 0 : configVariable.declarations;
      declaration[0].id.typeAnnotation = type;
      const typeImport = j__default.importDeclaration(
        [j__default.importSpecifier(j__default.identifier("BlitzCliConfig"))],
        j__default.literal("blitz")
      );
      typeImport.importKind = "type";
      program.node.program.body.unshift(typeImport);
    }
    const exportConfig = j__default.exportNamedDeclaration(configVariable);
  ...
  if (codemod) {
    ...
    fs__namespace.writeFileSync(blitzServerPath, newSource);
  }
});
const customTsParser = {
  parse(source, options) {
    ...
    return babelParser__namespace.parser.parse(source, babelOptions);
  }
};
...
function getStatements(node) {
  return j__default.BlockStatement.check(node) ? node.body : [node];
}
function replaceConditionalNode(path2, templateValues) {
  ...
  if (derivedCondition) {
    j__default(path2).replaceWith(getStatements(path2.node.consequent));
  } else {
    if (path2.node.alternate) {
      j__default(path2).replaceWith(getStatements(path2.node.alternate));
    } else {
      j__default(path2).remove();
    }
  }
}
function replaceConditionalStatements(program, templateValues) {
  ...
  program.find(j__default.IfStatement, processEnvRequirements).forEach((path2) => replaceConditionalNode(path2, templateValues));
  program.find(j__default.ConditionalExpression, processEnvRequirements).forEach((path2) => replaceConditionalNode(path2, templateValues));
  return program;
}
function replaceJsxConditionals(program, templateValues) {
  program.find(j__default.JSXIdentifier, { name: "if" }).forEach((path2) => {
    var _a, _b;
    if (j__default.JSXOpeningElement.check(path2.parent.node)) {
      const conditionPath = j__default(path2.parent).find(j__default.JSXAttribute, { name: { name: "condition" } }).at(0);
      ...
      const innerElements = path2.parent.parent.node.children.filter(
        j__default.JSXElement.check.bind(j__default.JSXElement)
      );
      const consequent = innerElements[0];
      const alternate = innerElements[1] && ((_b = j__default(innerElements[1]).paths()[0]) == null ? void 0 : _b.node.children.filter(j__default.JSXElement.check.bind(j__default.JSXElement)));
      const result = useConsequent ? consequent : alternate;
      if (!result) {
        j__default(path2.parent.parent).remove();
      } else {
        j__default(path2.parent.parent).replaceWith(result);
      }
    }
  });
  return program;
...
class Generator extends events.EventEmitter {
  constructor(options) {
    ...
    this.fs = memFsEditor.create(this.store);
    this.enquirer = new enquirer__default();
    this.prettier = prettier__default;
    this.useTs = typeof this.options.useTs === "undefined" ? fs__namespace.existsSync(path__namespace.resolve("tsconfig.json")) : this.options.useTs;
  ...
  replaceConditionals(input, templateValues, prettierOptions = {}) {
    const source = j__default(input, { parser: customTsParser });
    const program = source.find(j__default.Program);
  ...
  process(input, pathEnding, templateValues, prettierOptions) {
    ...
    if (!this.useTs && tsExtension.test(pathEnding)) {
      templatedFile = ((_a = babel__namespace.transform(templatedFile, {
        configFile: false,
        plugins: [[babelTransformTypescript__default, { isTSX: true }]]
      })) == null ? void 0 : _a.code) || "";
    }
    ...
    return __async$c(this, null, function* () {
      ...
      for (let filePath of paths) {
        try {
          ...
          pathSuffix = path__namespace.join(this.getTargetDirectory(), pathSuffix);
  ...
  sourcePath(...paths) {
    if (this.sourceRoot.type === "absolute") {
      return path__namespace.join(this.sourceRoot.path, ...paths);
    } else {
  ...
  destinationPath(...paths) {
    return path__namespace.join(this.options.destinationRoot, ...paths);
  }
  run() {
    return __async$c(this, null, function* () {
      debug$1("Generator.run...", { options: this.options });
      if (!this.options.dryRun) {
        yield fs__namespace.ensureDir(this.options.destinationRoot);
...
const defaultConfig = {
  ...
  experimental: {
    ...
    cpus: Math.max(
      1,
      (Number(process.env.CIRCLE_NODE_TOTAL) || (os__default.cpus() || { length: 1 }).length) - 1
    ),
    plugins: false,
    profiling: false,
    isrFlushToDisk: true,
...
function loadConfigProduction(pagesDir) {
  ...
  try {
    const path$1 = path.join(pagesDir, "next.config.js");
    debug("Loading config from ", path$1);
    userConfigModule = eval("require")(path$1);
  } catch (e) {
...
const baseLogger = (options) => {
  ...
  globalThis._blitz_baseLogger = new tslog.Logger(__spreadValues({
...
const fetchAllVersions = (dependency) => __async$b(void 0, null, function* () {
  const res = yield got__default(`https://registry.npmjs.org/${dependency}`, {
    retry: { limit: 3 },
    timeout: 3e3,
    responseType: "json"
  }).json();
...
const fetchDistTags = (dependency) => __async$b(void 0, null, function* () {
  const res = yield got__default(`https://registry.npmjs.org/-/package/${dependency}/dist-tags`, {
    retry: { limit: 3 },
    timeout: 3e3,
    responseType: "json"
  }).json();
...
const logFailedVersionFetch = (dependency, fallback) => {
  console.warn(
    `Failed to fetch latest version of '${chalk__default.bold(dependency)}'`,
    fallback ? `Falling back to '${chalk__default.bold(fallback)}'` : ""
  );
};
  ...
  getTemplateValues() {
    return __async$7(this, null, function* () {
      return {
        ...
        username: yield username__default()
      };
    });
  }
  getTargetDirectory() {
  ...
  postWrite() {
    return __async$7(this, null, function* () {
      ...
      if (!this.options.skipGit) {
        const initResult = spawn__default.sync("git", ["init"], {
          stdio: "ignore"
        });
      ...
      const pkgJsonLocation = path.join(this.destinationPath(), "package.json");
      const pkg = fs.readJSONSync(pkgJsonLocation);
      console.log("");
      ...
      yield fs.writeJson(pkgJsonLocation, pkg, { spaces: 2 });
      if (!fallbackUsed && !this.options.skipInstall) {
        spinner.succeed();
        yield new Promise((resolve) => {
          var _a2, _b2, _c, _d;
          const logFlag = pkgManager === "yarn" ? "--json" : "--loglevel=error";
          const cp = spawn__default(pkgManager, ["install", logFlag], {
...
${data}`);
        ...
        const runLocalNodeCLI = (command) => {
          ...
          if (pkgManager2 === "yarn") {
            return spawn__default.sync("yarn", ["run", ...command.split(" ")]);
          } else if (pkgManager2 === "pnpm") {
            return spawn__default.sync("pnpm", command.split(" "));
          } else {
            return spawn__default.sync("npx", command.split(" "));
          }
        };
  ...
  commitChanges() {
    ...
    for (let command of commands) {
      const result = spawn__default.sync(...command);
      if (result.status !== 0) {
        commitSpinner.fail(
          chalk__default.red.bold(
...
function plural(input) {
  return pluralize__default.isPlural(input) ? input : pluralize__default.plural(input);
}
function singular(input) {
  return pluralize__default.isSingular(input) ? input : pluralize__default.singular(input);
}
...
class Model {
  ...
  toString() {
    return ast__namespace.printSchema({
      type: "schema",
...
const getTemplateRoot = (templateDir, fallback) => {
  ...
  const templatePath = path__namespace.join(templateDir, fallback.path);
  if (!fs__default.existsSync(templatePath)) {
    console.info(
...
class ModelGenerator extends Generator {
  ...
  write() {
    return __async$6(this, null, function* () {
      ...
      let schema;
      try {
        schema = ast__namespace.getSchema(this.fs.read(schemaPath));
      } catch (err) {
      ...
      let fields = (extraArgs.length === 1 && ((_b = extraArgs[0]) == null ? void 0 : _b.includes(" ")) ? (_c = extraArgs[0]) == null ? void 0 : _c.split(" ") : extraArgs).flatMap((input) => Field.parse(input, schema));
        ...
        try {
          if (model) {
            for (const field of fields)
              field.appendTo(model);
            this.fs.write(schemaPath, ast__namespace.printSchema(schema));
...
`
        ...
        ast__namespace.printSchema({ type: "schema", list: [model] }).split("\n").map(log.progress);
        console.log("\n");
      }
    });
  }
    ...
    return __async$6(this, null, function* () {
      ...
      if (shouldMigrate) {
        yield new Promise((res, rej) => {
          ...
          const child = spawn.spawn(prismaBin, ["migrate", "dev"], { stdio: "inherit" });
...
class PageGenerator extends Generator {
  ...
  postWrite() {
    return __async$3(this, null, function* () {
      yield new Promise((res, rej) => {
        const blitzBin = which__default(process.cwd()).sync("blitz");
        const child = spawn.spawn(blitzBin, ["codegen"], { stdio: "inherit" });
        child.on("exit", (code) => code === 0 ? res() : rej());
      });
    });
  }
...
exports.customTemplatesBlitzConfig = customTemplatesBlitzConfig;
exports.customTsParser = customTsParser;
exports.fetchAllVersions = fetchAllVersions;
exports.fetchDistTags = fetchDistTags;
exports.fetchLatestDistVersion = fetchLatestDistVersion;
exports.getBlitzDependencyVersion = getBlitzDependencyVersion;
