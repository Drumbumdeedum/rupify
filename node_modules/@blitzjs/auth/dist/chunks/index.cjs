...
const useAuthorizeIf = (condition, role) => {
  ...
  if (blitz.isClient && condition && !getPublicDataStore().getData().userId && mounted) {
    const error = new blitz.AuthenticationError();
    error.stack = null;
    throw error;
  }
  if (blitz.isClient && condition && role && getPublicDataStore().getData().userId && mounted) {
    const error = new blitz.AuthenticationError();
    error.stack = null;
...
const useRedirectAuthenticated = (to) => {
  ...
  if (blitz.isClient && getPublicDataStore().getData().userId && mounted) {
...
function getAuthValues(Page, props) {
  ...
  if (authenticate === void 0 && redirectAuthenticatedTo === void 0) {
    const layout = "getLayout" in Page && ((_a = Page.getLayout) == null ? void 0 : _a.call(Page, /* @__PURE__ */ React__default.createElement(Page, __spreadValues({}, props))));
    if (layout) {
...
function withBlitzAuthPlugin(Page) {
  const AuthRoot = (props) => {
    ...
    return /* @__PURE__ */ React__default.createElement(Page, __spreadValues({}, props));
  };
...
exports.backupAntiCSRFTokenToLocalStorage = backupAntiCSRFTokenToLocalStorage;
