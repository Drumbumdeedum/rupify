...
const resolveCwd__default = /*#__PURE__*/_interopDefaultLegacy(resolveCwd);
...
const getPackageJson = () => __async$1(void 0, null, function* () {
  ...
  if (fs.existsSync(pkgJsonPath)) {
    return fs$1.readJSON(pkgJsonPath);
  }
  return {};
});
...
const codegenTasks = () => __async(void 0, null, function* () {
  try {
    const nextDir = yield resolveCwd__default("next");
    const nextClientIndex = path.join(nextDir, "../..", "client", "index.js");
    ...
    if (nextVersion && nextVersion.startsWith("12")) {
      ...
      logging.log.success("Next.js was successfully patched with a React Suspense fix");
    ...
    logging.log.error(JSON.stringify(err, null, 2));
  }
  try {
    yield routesManifest.generateManifest();
    logging.log.success("Routes manifest was successfully generated");
    ...
    if (hasPrisma) {
      const foundPrismaClient = resolveFrom__default.silent(process.cwd(), "@prisma/client");
      const foundDotPrismaClient = resolveFrom__default.silent(process.cwd(), ".prisma");
      if (!foundPrismaClient || foundPrismaClient && !fs__default.existsSync(path.join(foundPrismaClient, "../../..", ".prisma")) || foundDotPrismaClient && !foundDotPrismaClient) {
        let prismaSpinner = logging.log.spinner(`Generating Prisma client`).start();
        const result = yield runPrisma.runPrisma(["generate"], true);
        if (typeof result === "object") {
          if (result.success) {
            prismaSpinner.succeed(logging.log.greenText("Generated Prisma client"));
          } else {
            prismaSpinner.fail();
            console.log("\n" + result.stderr);
            process.exit(1);
...
exports.codegenTasks = codegenTasks;
