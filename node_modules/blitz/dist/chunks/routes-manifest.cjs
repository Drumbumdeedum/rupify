...
const loadConfig = (pagesDir) => {
  ...
  try {
    const path$1 = path.join(pagesDir, NEXT_CONFIG_FILE);
...
function createPagesMapping(pagePaths, config) {
  ...
  const pages = pagePaths.reduce((result, pagePath) => {
    ...
    result[pageKey] = path.join(PAGES_DIR_ALIAS, pagePath).replace(/\\/g, "/");
    return result;
  }, {});
  ...
  return __async(this, null, function* () {
    ...
    for (let { filePath, route, type } of allRoutes) {
      ...
      if (/\.mdx$/.test(filePath)) {
        ...
        const fileContents = yield readFile(path.join(process.cwd(), filePath), {
          encoding: "utf-8"
        });
    ...
    yield fs$1.outputFile(path.join(dotBlitz, "index.js"), implementation, {
      encoding: "utf-8"
    });
    yield fs$1.outputFile(path.join(dotBlitz, "index-browser.js"), implementation, {
      encoding: "utf-8"
    });
    yield fs$1.outputFile(path.join(dotBlitz, "index.d.ts"), declaration, {
      encoding: "utf-8"
    });
  });
}
...
function startWatcher(pagesDir2 = "") {
  return __async(this, null, function* () {
    ...
    return new Promise((resolve) => {
      fs$1.readdir(pagesDir2, (_, files) => {
      ...
      let wp = webpackWatcher = new Watchpack__default({});
      wp.watch(
        [],
        topLevelFoldersThatMayContainPages.map((dir) => path.join(pagesDir2, dir)),
        Date.now()
      );
