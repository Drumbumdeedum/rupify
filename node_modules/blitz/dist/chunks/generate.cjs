...
function modelName(input = "") {
  return generator.singleCamel(input);
}
function modelNames(input = "") {
  return generator.pluralCamel(input);
}
function ModelName(input = "") {
  return generator.singlePascal(input);
}
function ModelNames(input = "") {
  return generator.pluralPascal(input);
}
const createCustomTemplates = () => __async(void 0, null, function* () {
  const continuePrompt = yield prompts__default({
    type: "confirm",
    name: "value",
  ...
  const templatesPath = yield prompts__default({
    type: "text",
    name: "value",
  ...
  yield generator.customTemplatesBlitzConfig(isTypeScript, templatesPathValue, true);
  logging.log.success(`\u{1F680} Custom templates path added/updated in app/blitz-server file`);
  ...
  logging.log.success(`\u{1F680} Custom templates created in ${templatesPathValue} directory`);
  process.exit(0);
});
const generatorMap = {
  ["all" /* All */]: [
    ...
    generator.QueriesGenerator,
    generator.MutationsGenerator,
    generator.ModelGenerator
  ],
  ["crud" /* Crud */]: [generator.MutationsGenerator, generator.QueriesGenerator],
  ["model" /* Model */]: [generator.ModelGenerator],
  ["pages" /* Pages */]: [generator.PageGenerator, generator.FormGenerator],
  ["queries" /* Queries */]: [generator.QueriesGenerator],
  ["query" /* Query */]: [generator.QueryGenerator],
  ["mutations" /* Mutations */]: [generator.MutationsGenerator],
  ["mutation" /* Mutation */]: [generator.MutationGenerator],
  ["resource" /* Resource */]: [generator.QueriesGenerator, generator.MutationsGenerator, generator.ModelGenerator],
...
const generate = () => __async(void 0, null, function* () {
  ...
  try {
    ...
    const isTypeScript = yield getIsTypeScript();
    const cliConfig = yield generator.customTemplatesBlitzConfig(isTypeScript);
    for (const GeneratorClass of generators) {
      const generator$1 = new GeneratorClass({
        ...
        ParentModel: ModelName(selectedParent),
        ParentModels: ModelNames(selectedParent),
        name: generator.uncapitalize(selectedModelName),
        Name: generator.capitalize(selectedModelName),
        dryRun: args["--dry-run"],
